...
export function calculateSettlement(expenseSplits, expenses, users) {
  ...
  expenses.forEach(expense => {
    const payerId = expense.paidByUserId; // PRISMA FIX: paid_by_user_id -> paidByUserId
    const amount = parseFloat(expense.totalAmount); // PRISMA FIX: total_amount -> totalAmount
    balances.set(payerId, (balances.get(payerId) || 0) + amount);
  });

  // Subtract money each user owes (from expense_splits table)
  expenseSplits.forEach(split => {
    const userId = split.userId; // PRISMA FIX: user_id -> userId
    const amount = parseFloat(split.amountOwed); // PRISMA FIX: amount_owed -> amountOwed
  ...
  // Sort by largest amount first to minimize total transactions
